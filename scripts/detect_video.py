"""Run a video file or camera through FrameSource + YoloDetector and draw the
detections for visual sanity-checking: save an annotated video, display it
live, or both.

Usage:
    python scripts/detect_video.py path/to/video.mp4 --output out.mp4
    python scripts/detect_video.py 0 --display --confidence 0.4
    python scripts/detect_video.py path/to/video.mp4 --output out.mp4 --classes person,car
"""
...
def _draw_detections(image_bgr, detections: list[Detection]):
    ...
    try:
        with FrameSource(source, frame_skip=args.frame_skip) as frame_source:
            ...
            for _ in range(args.num_frames):
                ...
                _draw_detections(image_bgr, detections)
                ...
                if args.display:
                    cv2.imshow("TRACE detections", image_bgr)
