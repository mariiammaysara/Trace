"""ZONE_EXITED event: built from geometry.zone's ZoneTransition output
(transition == "exited"). Debounce itself lives in geometry.zone.ZoneDetector
...
def build_event(track: Track, transition: ZoneTransition, camera_id: str) -> Event:
    if transition.transition != "exited":
        raise ValueError(f"expected an 'exited' transition, got {transition.transition!r}")
    return Event(
