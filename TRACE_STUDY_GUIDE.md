...
| Live/Video dashboard view — playback + synced overlay (Section 0, added in Phase 10.1) | `[IMPLEMENTED]` | `dashboard/src/components/LiveView.tsx` | camera `Select`, `<video>` streamed from `GET /videos/{id}/stream`, Play/Pause `Button`, seek `Slider`, fetches cameras/videos/objects/trajectories/events/zones/lines from the real API (no mock data) |
...
| App-level view navigation — Dashboard ↔ Analytics (Phase 10.2) | `[IMPLEMENTED]` | `dashboard/src/App.tsx` | Plain local `useState<'dashboard' \| 'analytics'>` switch, no router dependency; `Sidebar`'s "Cameras"/"Events" items remain inert placeholders (no corresponding view built yet) |
