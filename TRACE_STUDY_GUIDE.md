...
### Event types — meaning, inputs, logic, edge cases `[MIXED — see per-row status; the event engine itself (rule wiring, schema, sink) is still [PLANNED]]`

| Event | Meaning | Required input | Core logic | Key edge case |
|---|---|---|---|---|
| `LINE_CROSSED` | Object crossed a configured virtual line | consecutive centroid pair, line geometry | check if the segment between two consecutive points intersects the line, determine side-before/side-after for direction | fast objects can "teleport" past a thin line between frames — must check segment intersection, not just point-in-front/behind — **geometric detection `[IMPLEMENTED]`, see below; not yet wired to actually emit a `LINE_CROSSED` record anywhere** |
| `ZONE_ENTERED` / `ZONE_EXITED` | Object crossed a polygon boundary | centroid, zone polygon | point-in-polygon test, compare current vs previous frame's containment | object flickering at a zone edge can fire enter/exit repeatedly — needs debounce/hysteresis — **geometric detection `[IMPLEMENTED]`, see below; not yet wired to actually emit `ZONE_ENTERED`/`ZONE_EXITED` records anywhere** |
| `OVERSPEED` | Estimated speed exceeds a configured limit | estimated speed (Section 6), configured limit | simple threshold compare | noisy single-frame speed spikes must be smoothed before comparing, or false alerts spike — `[PLANNED]` |
| `STOPPED` | Object stationary beyond a threshold duration | velocity history | velocity below noise threshold sustained over N seconds | must distinguish "genuinely stopped" from tracker jitter (Section 4) — `[PLANNED]` |
| `SUDDEN_STOP` | Rapid deceleration | velocity/acceleration history | large negative acceleration within a short window | must use real elapsed time, not frame count, given variable FPS — `[PLANNED]` (Section 4's `MotionStep.acceleration` already computes the raw signal this would threshold; no threshold/event wired yet) |
| `LOITERING` | Present/stationary in an area beyond a threshold | dwell time, zone or general presence | dwell time exceeds configured threshold | requires deciding whether brief tracking loss resets the timer or not (see Failure Cases, Section 17) — `[PLANNED]` (Section 4's `DwellTracker` already computes dwell time; no threshold/event wired yet) |
| `OBJECT_APPEARED` | A new track was created | track lifecycle state | new `object_id` created this frame | distinguishing a genuinely new object from a re-identification after occlusion — `[PLANNED]` |
| `OBJECT_DISAPPEARED` | A track was terminated | track lifecycle state | track exceeds missed-frame timeout | premature termination on brief occlusion produces false disappear/reappear pairs — `[PLANNED]` |

### Where the geometric detection is implemented `[IMPLEMENTED — event emission/schema/sink below remains [PLANNED]]`

- `src/geometry/line_crossing.py` — `Line` (dataclass: `id`, `start`, `end`, pixel coords), `segments_intersect()` (standard orientation-based segment-intersection test, handling colinear/touching-endpoint edge cases correctly, not just the general crossing case), `detect_line_crossing(line, point_before, point_after) -> LineCrossing | None` (the pure per-pair function this table's "Core logic" column describes), `LineCrossingDetector` (stateful, one `.update(object_id, point) -> list[LineCrossing]` call per frame per object, checked against every configured line), `load_camera_lines(camera_id, configs_dir)`.
- `src/geometry/zone.py` — `Zone` (dataclass: `id`, `polygon`), `point_in_polygon()` (via `cv2.pointPolygonTest`, not a hand-rolled ray-cast), `ZoneDetector` (debounced, `.update(object_id, point, timestamp, frame_id) -> list[ZoneTransition]`), `load_camera_zones(camera_id, configs_dir)`.
- **Debounce approach, made explicit as requested**: consecutive-frame confirmation. A raw containment reading only flips `ZoneDetector`'s confirmed state — and only then produces a `ZoneTransition` — after `debounce_frames` consecutive frames agree with the new reading in a row (default 3, configurable per `ZoneDetector` instance). A single-frame flicker back to the old state resets that streak to zero. This was chosen over a spatial inner/outer buffer-polygon approach for being the simplest thing that solves the actual named failure mode (one configurable integer, not a second polygon plus a margin distance to also configure), and it's directly analogous to a hardware button debounce.
- **Configured per camera** in the same per-camera JSON used for homography (Section 5) — `configs/cameras/<camera_id>.json` gains optional `"lines"` and `"zones"` arrays alongside `"correspondences"`, each entry carrying an `id`/`name` and pixel coordinates (`{"id": ..., "start": [x,y], "end": [x,y]}` for a line; `{"id": ..., "polygon": [[x,y], ...]}` for a zone). `configs/cameras/demo.json` was extended with one illustrative example of each.
- **Both operate in pixel space**, not real-world/homography-corrected space — matching the centroid positions Phase 4's trajectories already track in pixel space. (Real-world line/zone coordinates would be a separate, not-yet-needed design decision.)
- Tests (`tests/test_line_crossing.py`, `tests/test_zone.py`) specifically cover this table's two named edge cases: a fast-moving synthetic point pair (200px vertical jump in one frame) that a naive proximity check would miss but `detect_line_crossing()` catches via real segment intersection; and a rapid in/out/in/out flicker sequence at a zone boundary that never accumulates `debounce_frames` consecutive same-state reads, confirming zero spurious transitions — plus a follow-up test confirming a genuinely sustained entry still gets confirmed normally afterward.
...
| Event Engine — actually emitting `LINE_CROSSED`/`ZONE_ENTERED`/`ZONE_EXITED` (or any other event type), event schema, event sink (Section 7) | `[PLANNED]` | — | the geometric detection above is not yet wired to write or return actual event records anywhere; that wiring, plus the remaining event types (`OVERSPEED`, `STOPPED`, `SUDDEN_STOP`, `LOITERING`, `OBJECT_APPEARED`/`DISAPPEARED`), is the real Phase 7 |
